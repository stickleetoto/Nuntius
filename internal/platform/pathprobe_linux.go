...
func (p *commandPathProbe) Ping(ctx context.Context, target string, count int, timeout time.Duration) (model.PingResult, error) {
	...
	for i := 1; i <= count; i++ {
		...
		out, elapsed, err := runCommand(probeCtx, "ping", "-n", "-c", "1", "-W", strconv.Itoa(seconds), target)
		cancel()
...
func (p *commandPathProbe) Trace(ctx context.Context, target string, maxHops int, timeout time.Duration) (model.TraceResult, error) {
	...
	if _, err := lookPath("tracepath"); err == nil {
		out, _, cmdErr := runCommand(ctx, "tracepath", "-n", "-m", strconv.Itoa(maxHops), target)
		result := parseTraceOutput(string(out), targetIP, "tracepath", maxHops)
		result.Target, result.StartedAt, result.DurationMS = target, started.UTC(), time.Since(started).Milliseconds()
		if cmdErr != nil && len(result.Hops) == 0 {
			return result, fmt.Errorf("tracepath failed: %w", cmdErr)
		}
		return result, nil
	}
	return model.TraceResult{Target: target, ResolvedIP: targetIP, StartedAt: started.UTC(), MaxHops: maxHops}, commandNotFound("traceroute/tracepath")
}
