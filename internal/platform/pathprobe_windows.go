...
func (p *commandPathProbe) Ping(ctx context.Context, target string, count int, timeout time.Duration) (model.PingResult, error) {
	...
	for i := 1; i <= count; i++ {
		...
		out, elapsed, err := runCommand(probeCtx, "ping", "-n", "1", "-w", strconv.Itoa(millis), target)
		cancel()
...
func (p *commandPathProbe) Trace(ctx context.Context, target string, maxHops int, timeout time.Duration) (model.TraceResult, error) {
	...
	if _, err := lookPath("tracert"); err != nil {
	...
	out, _, cmdErr := runCommand(ctx, "tracert", "-d", "-h", strconv.Itoa(maxHops), "-w", strconv.Itoa(millis), target)
	result := parseTraceOutput(string(out), targetIP, "tracert", maxHops)
	...
	if cmdErr != nil && len(result.Hops) == 0 {
		return result, fmt.Errorf("tracert failed: %w", cmdErr)
	}
	return result, nil
}
